...
START_GAME() {
  ...
  (( GAMES_PLAYED++ ))
   ...
   RES_UPDATE_GAMES=$($PSQL "UPDATE users SET games_played=$GAMES_PLAYED WHERE username='$USERNAME'")
  if [[ -z $BEST_GAME || $COUNT_TRIES -lt $BEST_GAME ]]
  then
   RES_UPDATE_GAMES=$($PSQL "UPDATE users SET best_game=$COUNT_TRIES WHERE username='$USERNAME'")
  fi
